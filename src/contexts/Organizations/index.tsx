...
export default function OrganizationsProvider(props: ParentProps) {
	const [organizations, setOrganizations] = createStore(
		structuredClone(DEFAULT_ORGANIZATIONS_DATA),
	);

	function handleLoad(offset = 0) {
		client.organization.getMany
			.query({ offset, limit: LOAD_LIMIT })
			.then((res) => {
				if (!organizations.active)
					setOrganizations("active", res.at(0)?.organization.id);
				setOrganizations("organizations", (s) => [...s, ...res]);
				if (res.length === LOAD_LIMIT) handleLoad(offset + LOAD_LIMIT);
				else {
					const index = organizations.organizations.findIndex(
						(o) => o.organization.id === organizations.active,
					);
					if (index === -1)
						setOrganizations(
							"active",
							organizations.organizations.at(0)?.organization.id,
						);
				}
			});
	}
